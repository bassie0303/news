(function() {
  ...
  // 既に開いている場合は閉じる
  if (document.getElementById('news-share-dialog')) {
    document.getElementById('news-share-dialog').remove();
    return;
  }
  ...
  const dialogHTML = `
    ...
    ">
      <div style="padding: 20px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
          <h2 style="margin: 0; font-size: 20px; color: #333;">📰 記事をシェア</h2>
        ...
        <div style="background: #f5f5f5; padding: 12px; border-radius: 8px; margin-bottom: 15px;">
          ...
          <p style="margin: 5px 0; font-size: 12px; color: #333; word-wrap: break-word;">
            <strong>タイトル:</strong> ${articleData.title}
          </p>
          <p style="margin: 5px 0; font-size: 12px; color: #333; word-wrap: break-word;">
            <strong>URL:</strong> ${articleData.url}
          </p>
          <div style="max-height: 100px; overflow-y: auto; background: white; padding: 8px; border-radius: 4px; margin-top: 8px;">
            <strong style="font-size: 12px;">抜粋:</strong>
            <p style="margin: 5px 0; font-size: 12px; color: #333;">${articleData.excerpt || '抜粋なし'}</p>
          </div>
        </div>
        
        <div style="display: flex; flex-direction: column; gap: 10px;">
          <button id="share-all" style="
            padding: 14px;
            border: none;
            border-radius: 6px;
            cursor: pointer;
    ...
    <div id="news-share-overlay" style="
      position: fixed;
      top: 0;
      left: 0;
      right: 0;
  ...
  function closeDialog() {
    document.getElementById('news-share-dialog')?.remove();
    document.getElementById('news-share-overlay')?.remove();
  }
  ...
  document.getElementById('share-x').addEventListener('click', async () => {
    ...
    showStatus('Xを開きました。投稿内容を確認して投稿ボタンをクリックしてください。', 'success');
  });
  ...
  document.getElementById('share-all').addEventListener('click', async () => {
    ...
    window.open('https://www.facebook.com/', '_blank');
    setTimeout(() => window.open('https://www.threads.com/', '_blank'), 500);
